...
- **Per edge** (one hub→hub from/to pair): if it's served by more than 5 distinct airlines, keep only the top 2 by route-representation and the bottom 2 — a couple of dominant carriers, a couple of long-tail ones, deliberately not a smooth middle sample. That's always at least 4 survivors whenever trimming happens at all; an edge at or under 5 airlines is left untouched.
- **Premium retention**: if none of the 4 kept airlines offers a premium cabin (`hasFirstClass` or `hasBusinessClass`) but the edge has one available among the ones just cut, it's added back in as a 5th survivor — premium options shouldn't disappear from an edge just because they weren't in the top/bottom slice.
